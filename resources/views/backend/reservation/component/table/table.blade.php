<table border="1" class="table mb-0 checkbox-all" id="datatable_1">
    ...
    <tbody>
        ...
        <tr class="{{ $data->status == 'pending' ? 'bg-warning bg-opacity-50' : '' }}">
            <td style="width: 16px;">
            ...
            <td>
                <ul>
                    <li>{{ __('messages.reservation.fields.full_name') }}: {{ $data->name ?? __('messages.system.no_data_available') }}</li>
                    <li>{{ __('messages.reservation.fields.email') }}: {{ $data->email ?? __('messages.system.no_data_available') }}</li>
                    <li>{{ __('messages.reservation.fields.phone') }}: {{ $data->phone ?? __('messages.system.no_data_available') }}</li>
                </ul>
            </td>
            ...
            <td>
                ...
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-reservation-id="{{ $data->id }}" data-guestsReservation="{{ $data->guests }}" data-bs-target="#exampleModal">
                    {{ __('messages.system.button.choose_table') }}
                </button>
                @else
                <ul>
                    <li>{{ __('messages.table.fields.name') }}: {{ $data->table->name[App::getLocale()] ?? __('messages.system.no_data_available') }}</li>
                    <li>{{ __('messages.table.fields.description') }}: {{ $data->table->description[App::getLocale()] ?? __('messages.system.no_data_available') }}</li>
                    <li>{{ __('messages.table.fields.capacity') }}: {{ $data->table->capacity ?? __('messages.system.no_data_available') }}</li>
                </ul>
                @endif
            </td>
            <td>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-reservation-id="{{ $data->id }}" data-bs-target="#exampleModal">
                    @if ($data->table_id == null)
                    {{ __('messages.system.button.choose_dish') }}
                    @endif
                </button>
            </td>
            <!-- <td class="text-end">
